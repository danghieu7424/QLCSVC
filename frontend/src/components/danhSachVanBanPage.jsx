  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/update/danh-sach-thanh-ly`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
...
export default function DanhSachVanBanPage() {
  ...
  const updateVanBan = async (data) => {
    console.log("Updating vanBanList:", data[0]);
    try {
      const res = await fetch(`${API_BASE_URL}/api/update/danh-sach-van-ban`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      ...
      if (window.toast) window.toast.error("Cập nhật thất bại!");
    }
  };

  const handleUpdate = (rowIndex, keys, values) => {
    const updatedList = vanBanList.map((item, idx) =>
      idx === rowIndex
        ? {
            ...item,
    ...
    setVanBanList(updatedList);
    updateVanBan(updatedList);
  };
