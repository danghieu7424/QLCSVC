...
const pool = mysql.createPool({
    host: process.env.qlcsvc_DB_HOST,
    user: process.env.qlcsvc_DB_USER,
    password: process.env.qlcsvc_DB_PASSWORD,
    database: process.env.qlcsvc_DB_NAME,
...
(async () => {
    try {
        ...
        console.log("✅ Đã kết nối đến MySQL:", process.env.qlcsvc_DB_NAME);
        connection.release();
    } catch (err) {
