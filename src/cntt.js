...
function TaiSanCNTT() {
    ...
    useEffect(() => {
        // Gọi API
        fetch("https://api-jwgltkza6q-uc.a.run.app/api/ts_cntt")
            .then((response) => {
                if (!response.ok) {
                    throw new Error(`HTTP error! Status: ${response.status}`);
                }
...
function TLTaiSanCNTT() {
    const [data, setData] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        // Gọi API
        fetch("https://api-jwgltkza6q-uc.a.run.app/api/tlts_cntt")
            .then((response) => {
                if (!response.ok) {
                    throw new Error(`HTTP error! Status: ${response.status}`);
                }
...
export { PhongCNTT, TaiSanCNTT, TLTaiSanCNTT };
